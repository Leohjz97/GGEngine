...
#define VERTEX_FORMAT(fmt)\
	...
	struct VertexPC
	{
		...
		constexpr VertexPC(const GGMath::Vector3& position, const Color& color)
			: position(position)
			, color(color)
		{}
	...
	struct Vertex
	{
		...
		GGMath::Vector3 position;
		GGMath::Vector3 normal;
		GGMath::Vector3 tangent;
		GGMath::Vector2 uvCoord;
